 ...
 */
 ...
 (function ($) {
    ...
    this.getRemote = function(){
      ...
      _this.getRemote();
 			if (data) {
 				if(_this.currentCount != leaderlist.length){
 					_this.buildElements(_this.$content,leaderlist.length);
 				}
 				_this.currentCount = leaderlist.length;
 				_this.data = leaderlist;
 	...
 	Leaderboard.prototype.buildElements = function($ul,elemSize){
 		...
 		for (var i = 0; i < elemSize; i++) {
 			...
 			this.list.push({
                $item: item,
                $member: $('<span class="name">Loading...</span>').appendTo(item),
                $score: $('<span class="count">Loading...</span>').appendTo(item)
            });
 		}

 		function eventAnimationEnd (evt){
 			this.list[this.currentItem].$member.text(_this.data[this.currentItem].member);
